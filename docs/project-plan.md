# ClaimLens — 4-Week Build Plan (v2, corrected)

**Deployed on GCP, orchestrated through Claude Code, genuinely $0 out of pocket.**
...
**Day 15–16 — Hub-and-spoke multi-agent**
...
**Day 18–19 — Deterministic hooks — the centrepiece**
...
**Day 24 — Scheduled operations — the local version**
...
2. **`docs/decisions.md`** — ADR log: why Haiku for classification, why hub-and-spoke, why hooks over prompts, **why the local-Claude-Code / GCP-dashboard split instead of a single cloud service** — a genuinely good architecture decision worth documenting in its own right
...
## 5. After v1 — scaling to future projects

- **Project 2 — "SupplyLens"**: closes a gap in low-code agent tooling — built with Google's Agent Development Kit on Vertex AI Agent Engine, one agent authored in a low-code builder
- **Project 3 — "RepoWarden"**: an enterprise multi-agent code-review orchestrator, reuses the orchestrator/hooks/eval harness from this project
...
| Local machine is off when a scheduled run should happen | Note this limitation honestly in the README rather than hiding it |
